...
if ( ! function_exists( 'sptheme_setup' ) ) :
	...
	function sptheme_setup() {
		...
		register_nav_menus( array(
			...
			'menu-2' => esc_html__( 'Top Header Menu', 'maia' ),
			'menu-3' => esc_html__( 'Footer Menu', 'maia' ),
		) );
...
function cd_customizer_css()
{
    ?>
	 <style type="text/css">
		 #sp_theme-header .header-inner{ background: <?php echo get_theme_mod('header_background_color', '#43C6E4'); ?>; }
	 </style>
    <?php
}
 ...
 */
function maia_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'maia_content_width', 640 );
}
add_action( 'after_setup_theme', 'maia_content_width', 0 );
 ...
 */
function maia_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'maia' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'maia' ),
...
add_action( 'widgets_init', 'maia_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function maia_scripts() {
	wp_enqueue_style( 'maia-style', get_stylesheet_uri() );
	
	wp_enqueue_style( 'bootstrap-style', get_template_directory_uri().'/css/bootstrap.css' );
	
	wp_enqueue_script( 'bootstrap-js', get_template_directory_uri() . '/js/bootstrap.js', array(), '', true );

	wp_enqueue_script( 'maia-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'maia-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'maia_scripts' );
